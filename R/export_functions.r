...
#' @param maxIter numeric vector of length 1 specifying the maximal number of interations that should be
#' make while trying to protect common cells of two different tables. The
#' default value of parameter \code{maxIter} is 10.
...
#' result <- protectLinkedTables(problem1, problem2, commonCells, verbose=TRUE)
#'
...
#' @author Bernhard Meindl \email{bernhard.meindl@@statistik.gv.at}
protectLinkedTables <- function(objectA, objectB, maxIter=10, commonCells, ...) {
  ...
  paraList <- genParaObj(selection='control.secondary', ...)
      ...
      for (j in 1:2) {
        if (length(index[[j]]) > 0) {
          if (j == 1) {
            ...
            s_startJ(outA) <- 1
            s_startI(outA) <- 1
            outA <- c_quick_suppression(outA, input=paraList)
          } else {
            pI.B <- g_problemInstance(outB)
            ...
            s_startJ(outB) <- 1
            s_startI(outB) <- 1
            outB <- c_quick_suppression(outB, input=paraList)
          }
        }
      }

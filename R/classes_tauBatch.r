...
definition=function(obj) {
  ...
  cmds <- append(cmds, paste("<LOGBOOK>", dQuote(gsub("/data/home/meindl","Y:", file.path(path, obj@logbook)))))
  if (is_table) {
    cmds <- append(cmds, paste("<OPENTABLEDATA>", dQuote(gsub("/data/home/meindl","Y:", file.path(path, obj@microdata)))))
  } else {
    cmds <- append(cmds, paste("<OPENMICRODATA>", dQuote(gsub("/data/home/meindl","Y:", file.path(path, obj@microdata)))))
  }
  cmds <- append(cmds, paste("<OPENMETADATA>", dQuote(gsub("/data/home/meindl","Y:", file.path(path, obj@metadata)))))

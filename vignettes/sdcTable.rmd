...
Since versions `>0.23`, **sdcTable** contains some helper functions that allow an easier way to create the hierarchies. The function `create_node()`, `add_nodes()` and `delete_nodes()` make use of a tree-like data structure available from package [**data.tree**](https://CRAN.R-project.org/package=data.tree). We now show an alternative way to generate the hierarchy for variable `V1`.
...
**sdcTable** will internally convert the tree-based structure generated using `create_node()`, `add_nodes()` and `delete_nodes()` automatically into the `data.frame` based structure discussed at the begin of this section. What is nice using this dynamic approach to generate hierarchies is that there is a nice `plot` method for vizualization available:
